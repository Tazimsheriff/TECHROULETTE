...
from .models import TwinState, Batch, SensorPayload, WokwiTelemetryPayload, SimulationRequest, TickRequest
...
@app.post("/api/wokwi/telemetry", response_model=TwinState)
def receive_wokwi_telemetry(payload: WokwiTelemetryPayload):
    """
    Directly ingests real-time serial or network telemetry generated by the WOKWI ESP32 sketch.
    Maps physical sensors (DHT22 room temp/humidity, DS18B20 core fruit temp, battery, solar, buttons)
    to update the FreshVault digital twin state.
    """
    global CURRENT_STATE
    now_str = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

    updates = {
        "temperature": round(payload.roomTemperatureEffective, 1),
        "humidity": round(payload.humidity, 1),
        "lastUpdated": now_str,
    }
    if payload.solarWatts is not None:
        updates["solarPower"] = round(payload.solarWatts, 1)
    if payload.batteryPercent is not None:
        updates["batteryPercent"] = round(payload.batteryPercent, 1)
    if payload.doorOpen is not None:
        updates["doorOpen"] = payload.doorOpen
    if payload.coolingOn is not None:
        updates["coolingOn"] = payload.coolingOn
        updates["fanRpm"] = 1450.0 if payload.coolingOn else 0.0
        updates["compressorDutyCycle"] = 75.0 if payload.coolingOn else 0.0

    if payload.refrigerationFault is not None:
        if payload.refrigerationFault:
            updates["refrigeratorHealth"] = "failed"
            updates["activeAlert"] = "CRITICAL: Compressor thermal overload fault detected by Wokwi ESP32."
            updates["overallRisk"] = "critical"
            updates["activeSimulation"] = "cooling-failure"
        else:
            updates["refrigeratorHealth"] = "healthy"
            if updates.get("temperature", 6.0) <= 8.0:
                updates["overallRisk"] = "safe"
                updates["activeAlert"] = None

    if payload.risk is not None:
        r_lower = payload.risk.lower()
        if r_lower in ["safe", "warning", "critical"]:
            updates["overallRisk"] = r_lower

    if payload.recommendation:
        updates["systemRecommendation"] = payload.recommendation

    CURRENT_STATE = CURRENT_STATE.copy(update=updates)

    # Sync batch TOM-101 if batch details are sent by Wokwi
    target_id = payload.batchId or "TOM-101"
    for idx, b in enumerate(CURRENT_STATE.batches):
        if b.id.upper() == target_id.upper():
            batch_updates = {}
            if payload.qualityScore is not None:
                batch_updates["qualityScore"] = round(payload.qualityScore, 1)
            if payload.shelfLifeDays is not None:
                batch_updates["shelfLifeDays"] = round(payload.shelfLifeDays, 1)
            if payload.risk:
                r_lower = payload.risk.lower()
                if r_lower in ["safe", "warning", "critical"]:
                    batch_updates["risk"] = r_lower
            if payload.recommendation:
                batch_updates["recommendedAction"] = payload.recommendation

            # Add temperature point to history
            new_hist = list(b.temperatureHistory)
            time_str = datetime.now().strftime("%H:%M")
            new_hist.append({"timestamp": time_str, "temperature": round(payload.roomTemperatureEffective, 1)})
            if len(new_hist) > 12:
                new_hist = new_hist[-12:]
            batch_updates["temperatureHistory"] = new_hist

            CURRENT_STATE.batches[idx] = b.copy(update=batch_updates)
            break
